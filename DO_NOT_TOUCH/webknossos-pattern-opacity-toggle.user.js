// ==UserScript==
// @name         WK Pattern Opacity Toggle
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  Toggle Volume layer Pattern Opacity between 1 and saved value (Ctrl+Shift+P)
// @author       Bun
...
(function() {
    ...
    function log(...args) {
        if (DEBUG) console.log('[WK Pattern Opacity Toggle]', ...args);
    }
    ...
    function showNotification(message, type = 'info') {
        ...
        `;
        if (!document.querySelector('style[data-pattern-opacity-toggle]')) {
            style.setAttribute('data-pattern-opacity-toggle', 'true');
    ...
    function findVolumePatternOpacityInput() {
        log('Searching for Volume layer Pattern Opacity input...');
        ...
        for (const volumeLabel of allText) {
            ...
            // Look for "Pattern Opacity" label
            const patternOpacityLabels = Array.from(container.querySelectorAll('label.setting-label')).filter(el => {
                const text = el.textContent.trim();
                return text === 'Pattern Opacity';
            });

            if (patternOpacityLabels.length === 0) {
                log('No Pattern Opacity label found in this container');
                continue;
            }

            // Get the first Pattern Opacity label
            const patternOpacityLabel = patternOpacityLabels[0];
            log('Found Pattern Opacity label:', patternOpacityLabel);

            // Find the row containing this label
            let row = patternOpacityLabel.closest('.ant-row');

            if (!row) {
                log('No row found for pattern opacity label');
                continue;
            }
            ...
            if (numberInput) {
                log('Found pattern opacity number input:', numberInput);
        ...
        // Strategy 2: Search for any Volume layer and find Pattern Opacity input
        ...
        // Try to find the one that's in a Volume section with Pattern Opacity label
        for (const input of allInputs) {
            const row = input.closest('.ant-row');
            if (!row) continue;

            // Check if this row has "Pattern Opacity" label
            const label = row.querySelector('label.setting-label');
            if (label && label.textContent.trim() === 'Pattern Opacity') {
        ...
        log('Failed to find Volume pattern opacity input');
        return null;
    }
    ...
    function toggleOpacity() {
        log('=== Toggle Pattern Opacity Triggered ===');

        const opacityInput = findVolumePatternOpacityInput();

        if (!opacityInput) {
            showNotification('✗ Could not find Volume pattern opacity input', 'error');
            log('Pattern opacity input not found!');
            return;
        }

        const currentValue = parseInt(opacityInput.value);
        log(`Current pattern opacity value: ${currentValue}`);

        if (currentValue === 1) {
            // Toggle back to saved value
            const targetValue = savedOpacity || 40; // Default to 40 if no saved value
            setInputValue(opacityInput, targetValue);
            showNotification(`Pattern opacity → ${targetValue}`, 'success');
            log(`Toggled from 1 to ${targetValue}`);
        } else {
            // Save current value and toggle to 1
            savedOpacity = currentValue;
            setInputValue(opacityInput, 1);
            showNotification(`Pattern opacity → 1 (saved: ${savedOpacity})`, 'success');
    ...
    function setupKeyboardShortcut() {
        log('Setting up keyboard shortcut: Ctrl+Shift+P');
    ...
    function init() {
        ...
        log('='.repeat(50));
        log('WK Pattern Opacity Toggle v1.0.0');
        log('Keyboard Shortcut: Ctrl+Shift+P');
        ...
        showNotification('✓ Pattern Opacity Toggle loaded (Ctrl+Shift+P)', 'success');
    }
